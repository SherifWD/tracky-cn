...
class SubscriptionController extends Controller
{
    ...
    public function getAllTrackedShippings(Request $request)
    {
        ...
        if (!is_null($status)) {
            if ((int) $status === 3) {
                $query->where('saved_via_search', true);
            } else {
    ...
    public function searchShippment(Request $request)
    {
        ...
        $exists = ReservedShipping::where('track_number', $request->bill_no)
            ->where('carrier_code', $request->carrier_code)
            ->where('user_id', auth()->id())
            ->exists();
    ...
    public function getTrackedShippingByID(Request $request, $id)
    {
        $status = $request->query('status');
        ...
        if (!is_null($status)) {
            if ((int) $status === 3) {
                $shippings->where('saved_via_search', true);
            } else {
